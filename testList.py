...
def process_url(session, url_info):
    ...
    # 計算最大長度，並提供默認值以避免空序列錯誤
    max_length_ipv4 = max((len(str(entry)) for entry in sorted_entries if isinstance(entry, (ipaddress.IPv4Address, ipaddress.IPv4Network))), default=0)
    max_length_ipv6 = max((len(str(entry)) for entry in sorted_entries if isinstance(entry, (ipaddress.IPv6Address, ipaddress.IPv6Network))), default=0)
